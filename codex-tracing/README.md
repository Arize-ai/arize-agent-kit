...
2. **Notify hook** (`arize-hook-codex-notify`) -- Codex calls this after every agent turn. It builds an OpenInference LLM span from the turn payload, drains buffered events from the collector, assembles child spans (TOOL, CHAIN), and submits the complete span tree back to the collector for export.

When both the notify hook and native OTLP export are enabled, the notify hook builds a parent LLM span and attaches child spans from buffered event data, producing a rich trace tree per turn.
...
The installer guides you through configuration and supports interactive and non-interactive modes:
...
python3 install.py codex --target phoenix

# Non-interactive: Arize AX with native OTLP
python3 install.py codex --target arize --otlp
...
When Codex OTLP export is enabled, the notify hook drains buffered events for the current thread-id from the shared collector and builds child spans. These are merged with the parent span into a single OTLP `resourceSpans` payload, producing a hierarchical trace tree.
...
The shared collector exports spans to the configured backend (Phoenix or Arize AX). It is managed via `arize-collector-ctl`:

```bash
arize-collector-ctl start    # Start the shared collector
arize-collector-ctl stop     # Stop it
arize-collector-ctl status   # Check if running
```
...
```
codex-tracing/
  hooks/adapter.py             Adapter: thread-id state, debug dump, multi-span
  hooks/                       Notify hook entry point (Python CLI command)
...
```
core/common.py        State management (FileLock, StateManager)
core/collector.py     Shared collector: span export + event buffering
core/collector_ctl.py Collector lifecycle management
