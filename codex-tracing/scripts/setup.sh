...
if [[ -n "$existing_backend" ]]; then
  ...
  case "$existing_backend" in
    phoenix)
      phoenix_ep=$(jq -r '.backend.phoenix.endpoint // "http://localhost:6006"' "$SHARED_CONFIG")
      phoenix_key=$(jq -r '.backend.phoenix.api_key // empty' "$SHARED_CONFIG")
      cat > "$ENV_FILE" <<EOF
...
EOF
      ;;
    arize)
      arize_ep=$(jq -r '.backend.arize.endpoint // "otlp.arize.com:443"' "$SHARED_CONFIG")
      arize_key=$(jq -r '.backend.arize.api_key // empty' "$SHARED_CONFIG")
      arize_space=$(jq -r '.backend.arize.space_id // empty' "$SHARED_CONFIG")
      cat > "$ENV_FILE" <<EOF
# Arize Codex tracing environment (auto-generated)
export ARIZE_TRACE_ENABLED=true
