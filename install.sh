...
exec python3 "${collector_src}" "\$@"
BINEOF
    chmod +x "$SHARED_COLLECTOR_BIN"
    info "Installed collector runtime at ${SHARED_COLLECTOR_BIN}"
  else
  ...
  if ! command_exists python3; then
    warn "python3 not found — collector requires Python 3 to run"
...
# Arize Agent Kit — shared collector launcher
# Auto-generated by install.sh. Do not edit manually.
exec python3 "${INSTALL_DIR}/core/collector.py" "\$@"
BINEOF
    chmod +x "$SHARED_COLLECTOR_BIN"
...
uninstall() {
  ...
  # 4. Remove shared collector runtime files
